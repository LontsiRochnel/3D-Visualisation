...
let decalage = 800 // distance d'un object après l'écran avant d'être effacé
const movePx = 10
// ---Classes---
...
class Surface {
   ...
   */
  draw() {
    if (this.distance + decalage < camPos.screen && Math.abs(vectScal(camPos.direction, this.vect.unit)) > Math.cos(pi / 60)) {
      return
    }
    if (vectScal(pointsToVect(camPos.p, this.cent()), this.vect) == 0 || this.distance > 10000 || (vectScal(pointsToVect(camPos.p, this.a), camPos.direction) < 0 && this.a != undefined)) {
      return
    }
    ...
    let p = newPoint()
    let firstOutScreen = false // Pour voir si le premier point est hors ecran
    ...
    if (this.stroke == undefined || this.stroke) {
      if (this.stroke != undefined)
        paint.strokeStyle = this.stroke
      paint.stroke()
    }
    if (this.color == "none")
      return
    if (vectScal(pointsToVect(camPos.p, this.cent()), this.vect) < 0)
      paint.fillStyle = this.color[0]
    else
      paint.fillStyle = this.color[1]
    paint.fill()
  }
  *givePoints() { }
}
...
class Polygone extends Surface {
  ...
  *givePoints() {
    ...
    for (let i = 0; i <= this.n; i++) {
      ang = 2 * pi * i / this.n
      yield rotatePoint(this.cent(), vectTrans(this.center, vectSum(base[0].newLen(this.len * Math.cos(ang)), base[1].newLen(this.len * Math.sin(ang)))), this.rotate, base)
...
class InfiniteFlatSurface extends Surface {
  constructor(o = newPoint(), vectFromPos = newVect(0, 0, -100), base = [newVect(1, 0, 0), newVect(0, 1, 0), newVect(0, 0, 1)], color = "darkgreen") {
    super()
    this.center = o
  ...
  *givePoints() {
    let vectY = projectVector([this.baseOrigin[0], this.baseOrigin[1]], camPos.direction)
    this.base = [vectProd(vectY, this.baseOrigin[2]).unit, vectY.unit, this.baseOrigin[2].unit]
    let ang = 0
...
class SurfaceObject {
  constructor(...surfaces) {
    ...
    for (const surface of surfaces) {
      if (surface instanceof SurfaceObject)
        this.surfaces = this.surfaces.concat(surface.draw())
      else
...
class SurfaceDivided extends SurfaceObject {
  constructor(a = newPoint(), numX = 1, numY = 1, lenX = 0, lenY = 0, vect = newVect(0, 1), color = [newColTab()], stillDivide = false,) {
    ...
    for (let i = 0; i < color.length; i++) {
      ...
      if (color[i].num == 0)
        for (let c = this.colors.length; c < numX * numY; c++)
  ...
  draw() {
    if (this.surfaces[0] instanceof Surface)
      return this.surfaces
    else {
...
class SurfaceJoinObject extends SurfaceObject {
  ...
  getSurfaces() {
    ...
    for (let i = 0; i < points1.length; i++) {
      ...
      this.surfaces.push(new SurfaceLine(points1[i], this.color[i], pointsToVect(points1[i], points2[i]), pointsToVect(points2[i], points2[(i + 1) % points1.length]), pointsToVect(points2[(i + 1) % points1.length], points1[(i + 1) % points1.length]), pointsToVect(points1[(i + 1) % points1.length], points1[i])))
    }
  }
}
...
class CubeObject extends SurfaceObject {
  ...
  constructor(a = newPoint(), len = 100, color = ["blue", "red"], vectY = newVect(), vectZ = newVect()) {
    ...
    let left = new SurfaceLine(vectTrans(a, this.base[1].normed(-1)), this.colors[1], this.base[1], this.base[2].normed(-1), this.base[1].normed(-1), this.base[2])
    ...
    let behind = new SurfaceLine(vectTrans(left.a, this.base[0]), this.colors[3], this.base[0].normed(-1), this.base[2].normed(-1), this.base[0], this.base[2])
    let top = new SurfaceLine(vectTrans(a, this.base[1].normed(-1)), this.colors[4], this.base[0], this.base[1], this.base[0].normed(-1), this.base[1].normed(-1))
    let bottom = new SurfaceLine(vectTrans(a, this.base[2].normed(-1)), this.colors[5], this.base[0], this.base[1].normed(-1), this.base[0].normed(-1), this.base[1])
...
function vectScal(u = newVect(), v = newVect()) {
  return (u.coords.map((value, ind, a) => { return value * v.coords[ind] })).reduce((a, b, ind, ar) => { return a + b })
}
...
function projectVector(base = [newVect(), newVect()], vect = newVect()) {
  ...
  let proj2 = base[1].newLen(vectScal(base[1].unit, vectSub(vect, proj1)))
...
function projectPoint(p = newPoint()) {
  if (newProjectMode)
    return projectPoint2(p)
  else
    return projectPoint1(p)
}
...
function rotateVect(vect = newVect(), angle = 0, base = [newVect(1, 0, 0), newVect(0, 1, 0)]) {
  ...
  if (Math.abs(result.x) < 1e-10)
    result.x = 0
  if (Math.abs(result.y) < 1e-10)
    result.y = 0
  if (Math.abs(result.z) < 1e-10)
    result.z = 0
  return result
}

function getBaseFromVectZ(vect = newVect()) {
  if (vectProd(newVect(0, 0, 1), vect).norm == 0)
    return [newVect(1, 0, 0), vectProd(vect, newVect(1, 0, 0))]
...
let mouseControlClick = true, mouseControl = false, mouseDown = false
...
can.addEventListener('mousemove', (e) => {
  if (mouseControlClick && !mouseDown && !pointerLocked)
    return
  if (!mouseControl && !mouseControlClick)
    return
  if (lastMousePos[0] == -1) {
  ...
  if (Math.abs(camPos.angX) > pi * 0.9 / 2)
    camPos.angX = Math.sign(camPos.angX) * pi * 0.9 / 2
...
let d0 = new SurfaceRect(newPoint(100, 100, 350 + ref), ["dodgerblue", "dodgerblue"], newVect(25).normed(5), newVect(0, 0, -70).normed(5)),
  ...
  d2 = new SurfaceLine(newPoint(100, 100, 500 + ref), ["dodgerblue", "dodgerblue"], newVect(100, 0, 50).normed(5), newVect(100, 0, -50).normed(5), newVect(-200).normed(5));

let devant = new SurfaceObject(d0, d1, d2, d3, d4)
devant.stroke = "dodgerblue"

let derriere = new SurfaceLine(newPoint(100, 1100, ref), ["red", "red"], newVect(0, 0, 100).normed(5), newVect(100, 0, 50).normed(5), newVect(100, 0, -50).normed(5), newVect(0, 0, -100).normed(5), newVect(-200).normed(5))
...
world3D.addObject(devant, murGauche, murDroit, derriere, toitGauche, toitDroit)
